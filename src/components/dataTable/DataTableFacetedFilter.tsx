...
interface DataTableFacetedFilterProps<TData, TValue, OValue> {
  ...
  title: string;
  disabled?: boolean;
  options: { label: ReactNode; value: OValue; key: Key }[];
}
              ...
              {options.map((option) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={cn(
                        "flex size-3 items-center justify-center rounded-lg border",
                        isSelected
                    ...
                    {facets?.get(option.value) && (
                      <span className="text-muted-foreground ml-auto flex size-4 items-center justify-center font-mono text-sx">
                        {facets.get(option.value)}
                      </span>
                    )}
                  </CommandItem>
