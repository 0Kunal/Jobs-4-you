  ...
  inngest.createFunction(
    ...
    async ({ step, event }) => {
      const getUsers = step.run("get-user-settings", async () => {
        return db.query.OrganizationUserSettingsTable.findMany({
          where: eq(
      ...
      const getApplications = step.run("get-recent-applications", async () => {
        return db.query.JobListingApplicationTable.findMany({
          where: and(
            gte(
      ...
      const events = Object.values(groupedNotifications)
        .map((settings) => {
          ...
          return {
            ...
            data: { applications: filterApplications },
          } as const satisfies GetEvents<
            typeof inngest
  ...
  async ({ event, step }) => {
    ...
    await step.run("send-email", async () => {
      await resend.emails.send({
        ...
        react: DailyApplicationEmail({ applications, userName: user.name }),
      });
    });
  }
);
