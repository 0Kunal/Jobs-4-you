...
async function ApplyButton({ jobListingId }: { jobListingId: string }) {
  ...
  const application = await getJobListingApplication({ userId, jobListingId });
  if (application != null) {
    const fromatter = new Intl.RelativeTimeFormat(undefined, {
      style: "short",
      numeric: "always",
    });
    await connection();
    ...
    return (
      <div className="text-muted-foreground text-sm">
        ...
        {difference === 0 ? "today" : fromatter.format(difference, "days")}
      </div>
    );
  }
